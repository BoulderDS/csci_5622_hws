...
class VariationalBayes:
    ...
    def m_step(self, topic_counts):
        ...
        beta = numpy.zeros((self._num_topics, self._num_types))
        for ii in range(self._num_topics):
            self._beta[ii, ] = topic_counts[ii, ] / sum(topic_counts[ii, ])
        return self._beta
    ...
    def update_alpha(self, current_alpha=None, gamma=None):
        ...
        N = float(len(gamma))
        logphat = sum(self.dirichlet_expectation(g) for g in gamma) / N
        

        current_alpha =  numpy.ones(len(logphat)) * current_alpha
        new_alpha = self.update_dir_prior(current_alpha, N, logphat)
...
if __name__ == "__main__":
    ...
    # You need to prepare 20newsgroup dataset for the problem
    # In other words, you need to tokenize the dataset.
    # Also, to construct the vocab, please remove all stopwords (You can use NLTK for both tokenizing and filtering stopwords).
